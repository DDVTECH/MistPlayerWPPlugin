...
Tags: 'media server' 
Requires at least: 1.0??? change to wp version with shortcode
Tested up to: 1.0
Stable tag: 1.0
Requires PHP: 7.0
License: Unlicense
...
You can use from your wp-admin page in dashboard->settings->MistPlayer to set default values for all your posts. (there is an option for the loadbalancer).
You need to add your MistPlayer as a 'shortcode' element and there can set different individual settings. Stream name is a must. Example [mistplayer stream="bunny"].
Http value is a must if you haven't set up in the defaults page(settings->MistPlayer). Example [mistplayer stream="bunny" http="mistserver.org"]
...
Another useful feature is [mistplayer loadbalancer="http://ptv-web:8045"] 



You should always have in your individual post shortcode stream name. If you didn't set up the default settings, you have to add http as well, the rest is optional.
So, in order the individual post to work, you have to have minimum this as a 'shortcode' element: 
[mistplayer http="your_hostname" stream="your_stream"]
!Be careful: in Mistplayer default settings options's synthax is: {opt:1, opt:2} and when setting up the 
shortcode element for individual post, opt = '{width:500}'
